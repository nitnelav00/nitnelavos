...
class Window {
  ...
  Window(PVector position, GUIApp application) {
    ...
    draw(); // dessiner la fenêtre pour l'afficher
  }
  ...
  void update(boolean focus) {
    ...
    if (appli != null) { 
      ...
      affichage.beginDraw();
      appli.draw(affichage, taille); // dessinner le contenu dans l'image qui sera affichée plus tard
      affichage.endDraw();
    }
    ...
    if (mouseInRect(position.x + taille.x - 28*scaleMin, position.y, 28*scaleMin, 22*scaleMin)  && click && focus)
      detruire(id);
    // Détruire la fenêtre si elle sort de l'écran
